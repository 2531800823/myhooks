...
import { isString, isNumber, isFunction } from "../utils/index";
  ...
  useDeepCompareEffectWithTarget(
    () => {
      ...
      const callbackHandler = (e: KeyboardEvent) => {
        ...
        const keyGuard = genGuard(e);
        const firedKey = isValidKeyType(keyGuard) ? keyGuard : e.key;

        if (keyGuard) {
          return eventHandlerRef.current?.(e, firedKey);
        }
      };
